...
export default function GameStatePanel({
  ...
  const hasRunners = Boolean(game.bases.first || game.bases.second || game.bases.third);

  return (
    <aside className="game-state-panel" aria-label="現在の試合状況">
      <div className="game-state-visual">
      ...
      <div className="game-state-controls">
        ...
        <div className="game-action-buttons">
          <button type="button" disabled={!hasRunners || game.finished} onClick={onRunnerAction}>盗塁・走者プレー</button>
          <button type="button" disabled={!hasRunners || game.finished} onClick={onRunnerSubstitution}>走者交代（代走）</button>
          <button type="button" disabled={game.finished} onClick={onDefenseChange}>守備交代・位置変更</button>
        </div>
      </div>
    </aside>
  );
